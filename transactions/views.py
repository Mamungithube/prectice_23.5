...
from accounts.constants import DEPOSIT, WITHDRAWAL,LOAN, LOAN_PAID
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if recipient is not None and amount is not None:
                ...
                messages.error(request, 'Invalid recipient or amount.')
        else:
            messages.error(request, 'Invalid form data. Please check the input fields.')
        # send_transaction_email(self.request.user, amount , "Deposite Message",'deposite_email.html')
        return render(request, self.template_name, {'form': form})
